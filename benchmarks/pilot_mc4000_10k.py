"""Validate mc=4000 on full 10K SCOPe query set.

Compares mc=8000 (current) vs mc=4000 on full 10K queries.
"""
import json
import sys
import time
...
from benchmarks.benchmark_scop_search import (
    classify_hit, compute_roc_n, compute_average_precision, RankedHit,
)
...
def evaluate(hits_by_query, metadata):
    ...
    n = len(roc1_vals)
    return {
        "roc1": round(float(np.mean(roc1_vals)), 4) if n else 0,
        "roc5": round(float(np.mean(roc5_vals)), 4) if n else 0,
        "map": round(float(np.mean(map_vals)), 4) if n else 0,
    }
...
if __name__ == "__main__":
    ...
    print("Loading queries (full 10K)...", flush=True)
    query_ds = read_sequences(str(SCOP_DIR / "queries.fasta"), mode="protein")
    print(f"  {len(query_ds.records)} queries\n", flush=True)

    print(f"{'mc':>6} {'ROC1':>8} {'ROC5':>8} {'MAP':>8} {'Time':>8}")
    print("-" * 44)

    for mc in [8000, 4000]:
        metrics, elapsed = run_test(mc, metadata, db, query_ds)
        print(f"{mc:>6} {metrics['roc1']:>8.4f} {metrics['roc5']:>8.4f} "
              f"{metrics['map']:>8.4f} {elapsed:>7.1f}s", flush=True)
        import gc; gc.collect()

    print(f"\nReference: BLAST ROC1=0.8388, 589s")
