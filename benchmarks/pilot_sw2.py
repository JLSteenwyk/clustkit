#!/usr/bin/env python3
"""Pilot: SW with score-based filtering vs NW with identity filtering.

Fix: SW now passes all pairs with positive score (no identity threshold),
since the identity formula matches/shorter_length was too conservative
...
def run_config(db_index, query_ds, metadata, label, **kwargs):
    ...
    print(
        ...
        f"passing={len([h for qh in results.hits for h in qh]):>10d} "
        f"time={elapsed:.1f}s",
        flush=True,
    )
...
def main():
    ...
    print("=" * 140)
    print("SW (score-based filter) vs NW (identity filter)")
    print("=" * 140)
    ...
    configs = [
        # NW baseline
        {"label": "NW baseline (mc=5000)",
         **base, "max_cands_per_query": 5000, "local_alignment": False},

        # SW with score-based filtering, varying mc
        {"label": "SW score-filter (mc=3000)",
         **base, "max_cands_per_query": 3000, "local_alignment": True},
        {"label": "SW score-filter (mc=5000)",
         **base, "max_cands_per_query": 5000, "local_alignment": True},
        {"label": "SW score-filter (mc=8000)",
         **base, "max_cands_per_query": 8000, "local_alignment": True},

        # SW with pctl=99.0
        {"label": "SW score-filter (mc=5000, pctl=99.0)",
         **base, "freq_percentile": 99.0, "max_cands_per_query": 5000,
    ...
    # Save
    with open(out_dir / "sw2_pilot.json", "w") as f:
