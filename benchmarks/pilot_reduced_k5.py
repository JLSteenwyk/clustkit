#!/usr/bin/env python3
"""Pilot: reduced alphabet k=5 only."""
...
def main():
    ...
    print(">>> Dual index: std k=3 + reduced k=5 (mc=8000)", flush=True)
    ...
    results = search_kmer_index(
        ...
        max_cands_per_query=8000, reduced_alphabet=True, reduced_k=5,
    )
    ...
    print(f"\nRESULT: reduced k=5  ROC1={metrics['mean_roc1']:.4f}  "
          f"aligned={results.num_aligned}  time={elapsed:.1f}s", flush=True)
    print(f"\nReference: MMseqs2=0.7942  DIAMOND=0.7963  reduced k=4=0.7949", flush=True)
