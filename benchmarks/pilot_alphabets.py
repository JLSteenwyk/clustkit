#!/usr/bin/env python3
"""Pilot: multiple reduced alphabets for complementary homology detection.

Tests Dayhoff-6 (evolutionary grouping) and Hydrophobicity-8 (physical
property grouping) alongside the default Murphy-10 reduced alphabet.
Different alphabets capture different conservation patterns.
"""
...
from clustkit.kmer_index import (
    search_kmer_index, DAYHOFF6_ALPHA, DAYHOFF6_ALPHA_SIZE,
    HYDRO8_ALPHA, HYDRO8_ALPHA_SIZE,
)
...
def main():
    ...
    print("=" * 120)
    print("Multiple reduced alphabets")
    print("=" * 120)

    configs = [
        # Dayhoff-6 alone (replacing Murphy as extra alphabet)
        {"label": "Murphy + Dayhoff-6 k=[4,5]",
         **base, "extra_alphabets": [
             ("dayhoff", DAYHOFF6_ALPHA, DAYHOFF6_ALPHA_SIZE, [4, 5]),
         ]},

        # Hydrophobicity-8 alone
        {"label": "Murphy + Hydro-8 k=[4,5]",
         **base, "extra_alphabets": [
             ("hydro", HYDRO8_ALPHA, HYDRO8_ALPHA_SIZE, [4, 5]),
         ]},

        # Both extra alphabets
        {"label": "Murphy + Dayhoff + Hydro k=[4,5]",
         **base, "extra_alphabets": [
             ("dayhoff", DAYHOFF6_ALPHA, DAYHOFF6_ALPHA_SIZE, [4, 5]),
             ("hydro", HYDRO8_ALPHA, HYDRO8_ALPHA_SIZE, [4, 5]),
         ]},

        # All alphabets + our best spaced seeds
        {"label": "Murphy + Dayhoff + Hydro + spaced [11011,110011]",
         **base, "extra_alphabets": [
             ("dayhoff", DAYHOFF6_ALPHA, DAYHOFF6_ALPHA_SIZE, [4, 5]),
             ("hydro", HYDRO8_ALPHA, HYDRO8_ALPHA_SIZE, [4, 5]),
    ...
    with open(out_dir / "alphabets_pilot.json", "w") as f:
    ...
    for r in sorted(results, key=lambda x: -x["mean_roc1"]):
        print(f"  {r['label']:60s} ROC1={r['mean_roc1']:.4f} aligned={r['aligned']:>10d} time={r['time']:.1f}s")
    print(f"\n  Reference: Triple+IDF mc=8K = 0.8000")
