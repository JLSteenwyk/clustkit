#!/usr/bin/env python3
"""Pilot: spaced seeds on reduced alphabet for improved sensitivity.

Spaced seeds (e.g., 11011) match non-contiguous positions, tolerating
substitutions at "don't care" positions. Mathematically proven more
sensitive than contiguous seeds of the same weight.

Tests spaced seeds ADDED to the existing triple index pipeline.
"""
...
def main():
    ...
    base = dict(
        ...
        max_cands_per_query=8000,
    )

    print("=" * 120)
    print("Spaced seeds on reduced alphabet")
    print("=" * 120)

    configs = [
        # Baseline: triple index without spaced seeds
        {"label": "Triple k=[4,5] only (mc=8K)",
         **base, "reduced_alphabet": True, "reduced_k": [4, 5],
         "use_idf": True},

        # Add single spaced seed patterns (weight-4, various spans)
        {"label": "Triple + spaced 11011 (mc=8K)",
         **base, "reduced_alphabet": True, "reduced_k": [4, 5],
         "use_idf": True, "spaced_seeds": ["11011"]},

        {"label": "Triple + spaced 10101 (mc=8K)",
         **base, "reduced_alphabet": True, "reduced_k": [4, 5],
         "use_idf": True, "spaced_seeds": ["10101"]},

        {"label": "Triple + spaced 110011 (mc=8K)",
         **base, "reduced_alphabet": True, "reduced_k": [4, 5],
         "use_idf": True, "spaced_seeds": ["110011"]},

        # Multiple spaced seeds
        {"label": "Triple + spaced [11011,110011] (mc=8K)",
         **base, "reduced_alphabet": True, "reduced_k": [4, 5],
         "use_idf": True, "spaced_seeds": ["11011", "110011"]},

        # Best combo with higher mc
        {"label": "Triple + spaced [11011,110011] (mc=15K)",
         **base, "max_cands_per_query": 15000,
         "reduced_alphabet": True, "reduced_k": [4, 5],
         "use_idf": True, "spaced_seeds": ["11011", "110011"]},
    ]
    ...
    with open(out_dir / "spaced_seeds_pilot.json", "w") as f:
        json.dump(results, f, indent=2)

    print("\n" + "=" * 120)
    print("SUMMARY (sorted by ROC1)")
    print("=" * 120)
    print(f"  {'Config':55s} {'ROC1':>7s} {'Aligned':>10s} {'Time':>8s}")
    print("  " + "-" * 85)
    for r in sorted(results, key=lambda x: -x["mean_roc1"]):
        print(
            f"  {r['label']:55s} {r['mean_roc1']:7.4f} {r['aligned']:10d} "
            f"{r['time']:7.1f}s"
        )

    print(f"\n  Reference: MMseqs2=0.7942  DIAMOND=0.7963")
    print(f"  Previous best: Triple+IDF mc=15K ROC1=0.8082")
